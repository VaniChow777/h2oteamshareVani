...
DEFAULT_FIRST_UPSELL_BOOKMARK_ID = 23212
DEFAULT_REVENUE_BOOKMARK_ID = 23217


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare WebOB first-upsell funnels by conversion, ARPU, and SKU mix.")
    ...
    parser.add_argument(
        "--targets",
        nargs="+",
        help="One or more project:funnel_ids targets, such as bend:73,288 yoga:101,102. Use this for cross-project comparisons.",
    )
    parser.add_argument(
        "--target-periods",
        nargs="+",
    ...
    parser.add_argument("--first-upsell-bookmark-id", type=int, default=DEFAULT_FIRST_UPSELL_BOOKMARK_ID)
...
def parse_targets(args: argparse.Namespace) -> list[dict[str, str]]:
    ...
    seen_funnel_ids: dict[tuple[str, str, str], str] = {}
    for target in targets:
        funnel_id = target["funnel_id"]
        project = target["project"]
        key = (funnel_id, target["start"], target["end"])
        if key in seen_funnel_ids and seen_funnel_ids[key] != project:
            raise SystemExit(
                f"Funnel ID {funnel_id} appears under multiple projects in the same date window. "
                "The Sensors funnel report only exposes funnel_id, so duplicate IDs in the same window cannot be safely split."
            )
        seen_funnel_ids[key] = project
...
def sku_payment_counts(
    ...
    add_country_filter(payload, countries, country_field)
    payload["by_fields"] = [
        "event.view_deal_detail_wf_h2o.funnel_id",
    ...
    payload.update(
        {
            ...
            "filter": {
                "conditions": [
                    ...
                    {
                        "field": "event.$Anything.webob_product_type_is_double_extra",
                        "function": "equal",
                        "params": ["非二级增值"],
                    },
...
def write_report(path: Path, label: str, summary_rows: list[dict[str, Any]], detail_rows: list[dict[str, Any]], top_skus: int) -> None:
    lines = [f"# WebOB First-Upsell Funnel Compare - {label}", ""]
    ...
    for row in summary_rows:
        lines.append(
            "| {period} | {countries} | {project} | {funnel} | {page_users} | {submit_users} | {pay_users} | {submit_rate:.2f}% | {pay_rate:.2f}% | {overall_pay_rate:.2f}% | {estimated_revenue:.2f} | {arpu_by_page_user:.2f} | {arppu_by_pay_user:.2f} |".format(
                **row
            )
        )
    ...
    for summary in summary_rows:
        ...
        lines.append(f"### {summary['period']} {project} {funnel}")
        lines.append("")
        lines.append("| SKU | pay users | pay mix | avg revenue/order | est. revenue | revenue mix |")
        lines.append("|---|---:|---:|---:|---:|---:|")
        for row in rows[:top_skus]:
            lines.append(
                "| {sku_label} | {pay_users} | {pay_mix:.1f}% | {avg_revenue_per_order:.2f} | {estimated_revenue:.2f} | {revenue_mix:.1f}% |".format(
...
def main() -> int:
    ...
    first_upsell = get_bookmark(settings, args.dashboard_id, args.first_upsell_bookmark_id)
    ...
    for (start, end), window_targets in targets_by_window.items():
        ...
        rollups = funnel_rollups(settings, first_upsell, args.dashboard_id, start, end, funnel_ids, countries, args.country_field)
        payments = sku_payment_counts(settings, first_upsell, args.dashboard_id, start, end, funnel_ids, countries, args.country_field)
    ...
    for target in targets_sorted:
        ...
        summary_totals[target["target_key"]] = {
            "page_users": float(counts["page_users"]),
            "submit_users": float(counts["submit_users"]),
            "pay_users": float(counts["pay_users"]),
    ...
    for (target_key, sku), pay_users in payments_by_key.items():
        ...
        project = target["project"]
        funnel_id = target["funnel_id"]
        avg_prices = avg_prices_by_project_window[(project, target["start"], target["end"])]
        global_orders, global_revenue, avg_price = avg_prices.get(sku, (0, 0.0, money_from_sku(sku) or 0.0))
        ...
        detail_rows.append(
            {
                ...
                "sku_price_from_id": money_from_sku(sku),
            }
        )
        ...
        summary_rows.append(
            {
                ...
                "funnel": f"funnel{funnel_id}",
                "funnel_id": funnel_id,
                "page_users": int(page_users),
                "submit_users": int(submit_users),
                "pay_users": int(pay_users),
                "submit_rate": round(pct(submit_users, page_users), 4),
                "pay_rate": round(pct(pay_users, submit_users), 4),
                "overall_pay_rate": round(pct(pay_users, page_users), 4),
                "estimated_revenue": round(revenue, 2),
                "arpu_by_page_user": round(revenue / page_users, 4) if page_users else 0.0,
                "arppu_by_pay_user": round(revenue / pay_users, 4) if pay_users else 0.0,
            }
        )
    ...
    stem = f"first_upsell_{label}_{country_slug}_{target_slug}"
